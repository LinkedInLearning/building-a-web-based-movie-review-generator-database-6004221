  ...
  if (empty($errors)) {
    ...
    if ($is_edit) {
      ...
      foreach ($actor_ids as $actor_id) {
        $adminDB->addActorToMovie($movie_id, $actor_id);
      }
    } else {
      $movie_id = $adminDB->addMovie($data);
      foreach ($actor_ids as $actor_id) {
        $adminDB->addActorToMovie($movie_id, $actor_id);
      }
    }
...
<div class="container mt-4">
  <?php if ($success): ?>
    <div class="alert alert-success">Movie <?= $is_edit ? 'updated' : 'added' ?> successfully. Go back to the movies list to see it the new data.</div>
  <?php endif; ?>
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <?php if (!empty($movie['poster'])): ?>
        <img src="../uploads/posters/<?= htmlspecialchars($movie['poster']) ?>" alt="Poster"
          class="img-thumbnail d-block mb-2" style="max-height: 100px;">
      <?php endif; ?>
    ...
    <div class="mb-3">
      ...
      <input type="number" class="form-control" name="release_date" id="release_date"
        value="<?= htmlspecialchars($movie['release_date']) ?>" min="1900" max="<?= date('Y') + 5 ?>">
    </div>
